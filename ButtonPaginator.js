...
const paginator = async (msg, pages) => {
    ...
    if(!pages) throw new Error("Please provide a pages")
    let page = 0;
    const btn1 = new MessageButton().setEmoji("◀").setID("00001101001110").setStyle("blurple")
    const btn2 = new MessageButton().setEmoji("▶").setID("00101101001110").setStyle("blurple")
    const row = new MessageActionRow().addComponents([btn1, btn2])
    const btn1After = new MessageButton().setEmoji("◀").setID("00101101101110").setStyle("blurple")
    const btn2After = new MessageButton().setEmoji("▶").setID("00101101011110").setStyle("blurple")
    ...
    const col = await curPage.createButtonCollector(filter, { time: ms("60s") })
