...
router.get("/server/:guildID", checkAuth, async (req, res) => {
    ...
    if (!server && req.user.guilds.filter(u => ((u.permissions & 2146958591) === 2146958591)).map(u => u.id).includes(req.params.guildID)) {
        return res.redirect(`https://discord.com/oauth2/authorize?client_id=877177600307511386&permissions=8&redirect_uri=https%3A%2F%2Fagentdushield.tk%2Flogin&response_type=code&scope=bot&guild_id=${req.params.guildID}`);  //Full Permissions Invite
    } else if (!server) {
